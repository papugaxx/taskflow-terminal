...
export const TaskCard = ({ task, settings, onOpen, onAction }: Props) => {
  ...
  const backgroundStyle: CSSProperties = {
    ...
    backgroundImage: task.background?.imageUrl
      ? `url(${task.background.imageUrl})`
      : undefined,
    ...
    >
      ...
      <div className="task-card-content">
        <div className="task-card-meta">
          <Tag className="premium-tag priority-tag">
        ...
        {task.project && (
          <span className="task-project mono">{task.project}</span>
        )}
      ...
      <div className="task-card-footer">
        <span className="mono muted">
          {Math.round(task.estimateMinutes / 15) * 15} MIN ·{" "}
          {dayjs(task.updatedAt).format("DD.MM.YY")}
        </span>
        <Space>
          ...
          {task.status === "active" ? (
            ...
            <Tooltip title="Restore task">
              <Button
                ...
                size="small"
                icon={<UndoOutlined />}
                onClick={() => onAction(task.id, "restore")}
              />
            </Tooltip>
          )}
          <Popconfirm
          ...
          >
            <Button
              ...
              size="small"
              danger
              icon={deleteMode ? <DeleteOutlined /> : <InboxOutlined />}
            />
          </Popconfirm>
        </Space>
      </div>
