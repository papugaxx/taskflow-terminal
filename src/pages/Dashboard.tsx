...
type DashboardTab =
  "active" | "planner" | "completed" | "archive" | "analytics";
...
export const Dashboard = ({ settings, defaultTab = "active" }: Props) => {
  ...
  const [sortMode, setSortMode] = useState<
    "updated" | "priority" | "dueDate" | "estimate"
  >("updated");
  const [activeTab, setActiveTab] = useState<DashboardTab>(
    defaultTab === "analytics" ? "analytics" : defaultTab,
  );
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      plannedMinutes,
      workloadPercent: capacityMinutes
        ? Math.min(160, Math.round((plannedMinutes / capacityMinutes) * 100))
        : 0,
  ...
  const focusQueue = useMemo(() => {
    return activeTasks
      .sort((a, b) => {
        ...
        return (
          (a.dueDate ? dayjs(a.dueDate).valueOf() : Infinity) -
          (b.dueDate ? dayjs(b.dueDate).valueOf() : Infinity)
        );
      })
      .slice(0, 5);
  }, [activeTasks]);
  ...
  const projects = useMemo(() => {
    const names = [
      ...new Set(tasks.map((task) => task.project).filter(Boolean) as string[]),
    ];
  ...
  const projectData = useMemo(() => {
    const grouped = new Map<
      string,
      { active: number; minutes: number; completed: number }
    >();
    tasks.forEach((task) => {
    ...
    const maxCount = Math.max(
      ...
      ...days.map(
        (day) =>
          tasks.filter(
            (task) =>
              task.completedAt && dayjs(task.completedAt).isSame(day, "day"),
          ).length,
      ),
    );
    return days.map((day) => {
  ...
  const visibleTasks = useMemo(() => {
    ...
    return tasks
      .filter((task) => {
        ...
        if (
          projectFilter !== "all" &&
          (task.project || "Inbox") !== projectFilter
        )
          return false;
  ...
  const overdueValueStyle: CSSProperties = {
    color: stats.overdue ? "#f0b983" : undefined,
  };
  ...
  const handleAction = (id: string, action: Action) => {
    ...
    const updated = tasks
      .map((task) => {
        ...
        if (action === "archive")
          return { ...task, status: "deleted" as const, updatedAt: now };
        if (action === "delete") return null;
        if (action === "complete")
          return {
            ...task,
            status: settings.autoArchiveCompleted
              ? ("deleted" as const)
              : ("completed" as const),
            completedAt: now,
            updatedAt: now,
    ...
    setSelectedTask((current) =>
      current?.id === id
        ? (updated.find((task) => task.id === id) ?? null)
        : current,
    );
    message.success(
      action === "delete"
  ...
  const renderCards = (status: Task["status"]) => {
    ...
    if (!list.length)
      return <Empty className="empty-state" description="No tasks found" />;
    return (
      <Row gutter={[18, 18]}>
        {list.map((task) => (
    ...
    ? [
        ...
        ...(selectedTask.completedAt
          ? [
              {
                color: "green",
                children: `Completed ${dayjs(selectedTask.completedAt).format("DD MMM YYYY, HH:mm")}`,
              },
            ]
          : []),
      ]
  ...
  return (
    <div className="dashboard">
      <section className="hero-panel hero-panel--product liquid-panel">
        <div className="hero-content">
          ...
          <p className="hero-copy">
            A premium productivity cockpit for capacity planning, deep-work
            focus and project health — not another plain to-do list.
          </p>
        ...
        {settings.showFocusPanel && (
          <aside className="focus-queue-card">
            <div className="focus-queue-header">
              ...
              <div>
                <p className="mono muted">TODAY'S FOCUS</p>
                <strong>
                  {focusQueue.length ? "Next best moves" : "Clear queue"}
                </strong>
              </div>
            </div>
            <div className="focus-queue-list">
              {focusQueue.length ? (
                ...
                <p className="muted">
                  Create active tasks and the planner will build a queue.
                </p>
              )}
            </div>
          </aside>
      ...
      <Row gutter={[12, 12]} className="stats-grid product-stats">
        <Col xs={12} lg={6}>
          <Card className="stat-card liquid-panel" variant="borderless">
        ...
        <Col xs={12} lg={6}>
          <Card className="stat-card liquid-panel" variant="borderless">
            <Statistic
              title="Planned work"
              value={formatHours(stats.plannedMinutes)}
            />
          </Card>
        </Col>
        <Col xs={12} lg={6}>
      ...
      {stats.workloadPercent > 100 && (
        <Alert
          className="workload-alert"
          type="warning"
          showIcon
          message="Capacity warning"
          description={`You planned ${formatHours(stats.plannedMinutes)} of work for a ${settings.dailyCapacityHours}h daily capacity. Move or split tasks before the day becomes overloaded.`}
        />
      )}
      ...
      <div className="board-panel liquid-panel">
        <div className="toolbar">
          <Input
            allowClear
            placeholder="Search tasks, projects, tags or status"
        ...
        <Tabs
          ...
          items={[
            {
              key: "active",
              label: `Board (${stats.active})`,
              children: renderCards("active"),
            },
            {
              key: "completed",
              label: `Done (${stats.completed})`,
              children: renderCards("completed"),
            },
            {
              key: "archive",
              label: `Archive (${stats.deleted})`,
              children: renderCards("deleted"),
            },
            {
              key: "planner",
              label: "Planner",
              children: (
                <div className="planner-grid">
                  <Card
                    className="liquid-panel planner-card"
                    variant="borderless"
                  >
                    ...
                    <div className="capacity-hero">
                      ...
                      />
                      <div>
                        <h3>{formatHours(stats.plannedMinutes)}</h3>
                        <p className="muted">
                          planned against {settings.dailyCapacityHours}h
                          capacity
                        </p>
                      </div>
                    </div>
                  ...
                  >
                    ...
                    {projectData.map((item) => (
                      <div className="project-row" key={item.project}>
                        <b>{item.project}</b>
                        <span>
                          {item.active} active · {formatHours(item.minutes)}
                        </span>
                        <Progress
                          percent={Math.min(
                            100,
                            Math.round(
                              (item.minutes / Math.max(capacityMinutes, 1)) *
                                100,
                            ),
                          )}
                          showInfo={false}
                  ...
                  >
                    ...
                    <div className="feature-stack">
                      <div>
                        <FieldTimeOutlined />
                        <b>Effort estimates</b>
                        <span>
                          People see if the day is realistic before starting.
                        </span>
                      </div>
                      <div>
                        <FireOutlined />
                        <b>Energy matching</b>
                        <span>
                          Deep work, focus tasks and low-energy tasks are
                          separated.
                        </span>
                      </div>
                      <div>
                        ...
                        <b>Deadline queue</b>
                        <span>
                          The app recommends what to do next instead of just
                          storing tasks.
                        </span>
                      </div>
                    </div>
            ...
            },
            {
              ...
              children: (
                <div className="analytics-grid premium-analytics">
                  <Card
                    className="liquid-panel chart-card"
                    variant="borderless"
                  >
                    <p className="mono muted">PRIORITY LOAD</p>
                    {priorityData.map((item) => (
                      <div className="bar-row" key={item.priority}>
                        <span>{item.priority}</span>
                        <div className="bar-track">
                          <div
                            className={`bar-fill bar-${item.priority}`}
                  ...
                  <Card
                    ...
                    variant="borderless"
                  >
                    <p className="mono muted">7-DAY COMPLETION</p>
                    <div className="mini-chart">
                      {weeklyData.map((item) => (
                        <div className="mini-chart-column" key={item.label}>
                          <div
                  ...
                  <Card
                    className="liquid-panel chart-card smart-plan-card"
                    variant="borderless"
                  >
                    <p className="mono muted">SMART DAILY PLAN</p>
                    ...
                    <ol className="smart-plan-list">
                      ...
                      {!focusQueue.length && (
                        <li>Create active tasks to generate the queue.</li>
                      )}
                    </ol>
                    <p className="muted">
                      Auto-prioritizes urgent work and nearest deadlines.
                    </p>
                  </Card>
                  <Card
                    className="liquid-panel chart-card focus-card"
                    variant="borderless"
                  >
                    <p className="mono muted">FOCUS INDEX</p>
                    <Progress
                      type="circle"
                    ...
                    />
                    <p className="muted">
                      {stats.urgent} urgent · {stats.dueSoon} due soon ·{" "}
        ...
        footer={
          selectedTask
            ? [
                <Button
                  key="edit"
                  onClick={() => handleAction(selectedTask.id, "edit")}
                >
                  Edit
                </Button>,
                selectedTask.status === "active" ? (
                  <Button
                    key="complete"
                    type="primary"
                    onClick={() => handleAction(selectedTask.id, "complete")}
                  >
                    Complete
                  </Button>
                ) : (
                  <Button
                    key="restore"
                    type="primary"
                    onClick={() => handleAction(selectedTask.id, "restore")}
                  >
                    Restore
                  </Button>
                ),
      ...
      >
        {selectedTask && (
          <div className="task-detail">
            <div className="task-detail-hero liquid-panel">
              <Tag>{selectedTask.priority.toUpperCase()}</Tag>
              {selectedTask.project && <Tag>{selectedTask.project}</Tag>}
            ...
            <Descriptions bordered column={1} size="small">
              ...
              <Descriptions.Item label="Estimate">
                {selectedTask.estimateMinutes} minutes · {selectedTask.energy}
              </Descriptions.Item>
